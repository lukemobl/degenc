...
const CLIENT_ANIMATION_LENGTH = 13000;
...
const listen = io => {
  ...
  const startNewGame = async () => {
    ...
    // Emit to clients
    io.of("/shuffle").emit("new-round", GAME_STATE._id, GAME_STATE.privateHash);
  };
    ...
    socket.on("join-game", async betAmount => {
      ...
      try {
        ...
        const dbUser = await User.findOne({ _id: user.id });
                ...
                if (dbUser.selfExcludes.shuffle > Date.now()) {
                  return socket.emit(
                    "game-join-error",
                    `You have self-excluded yourself for another ${((dbUser.selfExcludes.shuffle - Date.now()) / 3600000).toFixed(1)} hours.`
                  );
                }
