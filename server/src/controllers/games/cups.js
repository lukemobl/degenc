...
const CLIENT_ANIMATION_LENGTH = 5000; //try around: original: 4000
      ...
      async (players, color, betAmount, private, costMultiplier) => {
        ...
        try {
          ...
          const dbUser = await User.findOne({ _id: user.id });
                    ...
                    if (dbUser.selfExcludes.cups > Date.now()) {
                      return socket.emit(
                        "game-join-error",
                        `You have self-excluded yourself for another ${((dbUser.selfExcludes.cups - Date.now()) / 3600000).toFixed(1)} hours.`
                      );
                    }
